    ...
    if (passwordField && toggle) {
        toggle.addEventListener('click', () => {
            ...
            if (formId === 'signup-form') {
                if(confirmPasswordInput) confirmPasswordInput.type = isPassword ? 'text' : 'password';
            }
...
if (signupForm) {
    signupForm.addEventListener('submit', async (e) => {
        ...
        [fullnameInput, emailInput, passwordInput, confirmPasswordInput].forEach(input => input.classList.remove('invalid'));
        ...
        if (password.length < 8 || !password.match(/[A-Z]/) || !password.match(/[0-9]/) || !password.match(/[^A-Za-z0-9]/)) {
            showError("Password is too weak. It must be at least 8 characters long and include an uppercase letter, a number, and a special character.");
            passwordInput.classList.add('invalid');
            return;
        }
        if (!terms) {
        ...
        try {
            ...
            localStorage.setItem('welcomeMessage', `Welcome, ${fullname}! Please check your email to verify your account.`);
